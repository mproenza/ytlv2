...
/*Modiffication for YoTeLlevoCuba by Yuniel Proenza May/13/2020*/
(function() {
  // Reference jQuery
  var $;
  ...
  $.awesomeCropper = function(inputAttachTo, options) {
    ...
    // Main box
    $container = $("#cropper").addClass('awesome-cropper');
    ...
    $("#uploader").append($fileSelect);//Modiffied
   ...
   /* $progressBar = div().addClass('progress hide').append(div().addClass('progress-bar').attr({
    ...
    $sourceIm = image()
    $sourceIm.attr('width','80%');
    $sourceIm.attr('heigh','80%');
    $applyButton = a('Aplicar').addClass('btn btn-primary btn-xs').attr('hidden',true);
    $cancelButton = a('Cancelar').addClass('btn btn-danger btn-xs').attr({
      'data-dismiss': "modal",
      'hidden':"true"
    });
    /*Some required modiffications here*/
    $imagesContainer = div().append(div().addClass('panel').append(div().addClass('panel-content').append(div().addClass('panel-header').append("<p>Recorte imagen a su gusto:</p>"),div().addClass('panel-body').append(div().addClass('col-md-9').append($sourceIm)).append(div().addClass('col-md-3').append($cropSandbox)), div().addClass('panel-footer').append(div().addClass('btn-group').append($cancelButton).append($applyButton)))));
    ...
    return $applyButton.on('click', function() {
      ...
      $("canvas").hide();//Added
      saveCrop();
      return true; //$imagesContainer.modal('hide'); 
    });
  };
